...
## Phase 13 CI boundary

GitHub Actions has top-level `contents: read` permission and uses pull-request events without `pull_request_target`. It requires no repository secrets, cloud API, registry login, private input, persistent developer volume, or deployment credential. PostgreSQL credentials and demo tokens in the workflow are explicitly disposable CI-only values. Expanded Compose configuration is not printed.

The only actions are GitHub-maintained checkout and Python setup actions pinned to immutable commit SHAs. Python caching is keyed by the tracked lock file and caches pip downloads rather than project runtime state. The workflow builds images without pushing them and checks that common ignored runtime artifacts are absent. It is a repository validation boundary, not a production supply-chain assurance program; vulnerability scanning, attestations, signing, dependency update automation, protected-branch configuration, and hosted execution evidence remain outside Phase 13.

## Phase 14 hardening boundary

Targeted regressions cover safe authentication and malformed-JSON responses, prompt injection in user, policy, and event text, the fixed read-only tool allowlist, graph/tool/result bounds, unsupported identifiers and ML claims, dependency failures, and two overlapping investigations. Application-owned evidence remains the source of event and incident IDs, policy citations, anomaly scores, and anomaly flags. Generated anomaly-flag prose that conflicts with typed ML evidence is now replaced with a neutral reference and marked in the response errors.

A practical scan covered every current tracked path and every blob reachable from local Git history. It found no committed environment file, high-confidence credential or private key, private policy/data directory, model artifact, MLflow database, runtime ground truth, temporary output, or macOS developer home path. Container-internal `/home/esic` paths and the Phase 0 record of the inspected Python installation are intentional. Git commit metadata contains one non-noreply author mailbox; a publisher who wants address privacy should choose a history policy before publishing. Phase 14 did not rewrite history.
